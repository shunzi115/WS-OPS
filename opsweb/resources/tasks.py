...
''' 阿里云服务器自动添加 '''
@shared_task(bind=True,name="ServerAliyunAutoAddCrontab")
def ServerAliyunAutoAddCrontab(self):
...
def ServerAliyunAutoRefreshCrontab(self):
    ...
    for id in id_list:
        try:
            ...
            celery_log_error.error("获取server_aliyun 对象失败,错误信息: %s" % (e.args))
            continue

        ret = GetServerInfoFromApi(private_ip, server_aliyun_obj)

        if ret["result"] == 1:
            celery_log_error.error("服务器: %s 自动刷新失败,错误信息: %s" % (private_ip, ret["msg"]))
            continue
        else:
            celery_log_info.info("服务器: %s 自动刷新成功" % (private_ip))


''' 服务器数量按天统计 '''
@app.task(name="ServerStatisticByDayCrontab")
def ServerStatisticByDayCrontab():
    ...
    try:
        ...
        celery_log_error.error("统计 %s 的服务器总数失败,错误信息: %s" % (myday, e.args))
        sys.exit()
    else:
...
def CmdbStatisticByDayCrontab():
    ...
    try:
        ...
        celery_log_error.error("统计 %s 的应用总数失败,错误信息: %s" % (myday, e.args))
        sys.exit()
    else:
        celery_log_info.info("统计 %s 的应用总数成功" % (myday))
...
def CmdbAutoAddCrontab():
    ...
    for ss in server_ip_name_list:
        try:
            ...
            celery_log_error.error("获取服务器 %s 对象失败,错误信息: %s" % (ss["private_ip"], e.args))
            continue

        if ss["status"] != "Running":
            celery_log_info.info("该服务器: %s 状态 %s 所以需要删除其关联的CMDB" % (ss["private_ip"], ss["status"]))
            ss_obj.cmdbmodel_set.clear()
            continue

        if not ss["instance_name"]:
            celery_log_error.error("该服务器: %s 没有实例名" % (ss["private_ip"]))
            continue

        app_name_list = ss["instance_name"].split("_")[-2::-1]
        if not app_name_list:
            celery_log_error.error("该服务器: %s 实例名 %s 格式不正确" % (ss["private_ip"], ss["instance_name"]))
            continue
        ...
        for app in app_name_list:
            ...
            if not cmdb_name_list:
                ...
                try:
                    ...
                    celery_log_error.error("CmdbModel 自动添加失败,错误信息: %s" % (e.args))
                    continue
                else:
                    celery_log_info.info("CmdbModel 自动添加对象 %s 并关联 ServerModel 成功" % (app))
                    continue
            ...
            if app in cmdb_name_list:
                try:
                    ...
                    celery_log_error.error("CmdbModel 查询对象 %s 失败,错误信息: %s" % (app, e.args))
                    continue
                ...
                try:
                    ...
                    celery_log_info.info("CmdbModel 自动添加对象 %s 成功" % (app))
                    ''' 每个模块的管理组都要关联 ops 组 '''
        ...
        try:
            ...
            celery_log_error.error("CmdbModel 自动删除关联IP为空的对象失败,错误信息: %s" % (e.args))
